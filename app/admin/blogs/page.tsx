      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {blogs.length === 0 ? (
            ...
            blogs.map((b: any) => (
              <div key={b.id} className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50/50 transition-colors">
                <div className="space-y-1.5 max-w-xl">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#E2C799] bg-[#1C2E26] px-2 py-0.5 rounded inline-block">
                    {b.category}
                  </span>
                  <h3 className="text-sm font-bold text-[#1C2E26]">{b.title}</h3>
                  <p className="text-xs text-gray-500 line-clamp-2">{b.excerpt}</p>
                </div>
